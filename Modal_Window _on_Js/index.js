...
document.addEventListener('click', event => {//рендер вызывает виртуальные карточки, для работы же с модальным окном , а у нас есть кнопки, нам нужно вызвать слушателя, который при наличие в виртуальном шаблоне дата аргументов , установленных для соответствующих модальных окон, легко определить, указав необходимые для него свойства:
    ...
    if (btnType === 'price') {        //при подтверждении окна цены мы через сеттаймаут меняем контекст окна цены
        ...
        $.confirm({//вызываем окно через новый плагин, который возвращает нам промис, который требует обработки методами then & catch
            title: 'Вы уверены?',
            content: `<p>Вы удаляете матку: <strong>${queen_bee.title}</strong></p>`
        }).then( () => {//в этом случае мы попадаем на кнопку Удалить.
            queen_bees = queen_bees.filter(f => f.id !== id)//добавляя же фильтр в данном методе мы удаляем и необходимый элемент массива., переопределяя конкретный элемент, для этого массив дожен быть переменной изменяемой , не const , а let, и дополнительно нужно вызвать рендер для обновления сосстояния карточек.
            render()
        }).catch( () => { // в этом случае на кнопку Отменаю
            console.log('Cancel')
        })
    }    
})
