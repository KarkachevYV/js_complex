...
class Animal {// классы записываются только с большой буквы
    ...
    constructor(options) { //для инициализации начальных полей объекта данного класса реализуется специальный метод constructor параметром которого будет некий оюъект options
    ...
    voice() {//мы можем создавать свои методы и передавать их по наследству
        console.log('I am Animal')
    }
}
//к примеру после объявления родительского класса мы можем создать дочерний объект через переменную
...
class Cat extends Animal {
    static type = 'Cat'
    constructor(options) {//и даже добавлять свои поля через конструктор, но делается это с обязательным вызовом конструктора родителя используя ключевое слово super с параметром options родителя
        super(options)
    ...
    set ageInfo(newAge) {
        this.age = newAge//newAge = cat.ageInfo -так создали новый возраст для дочерних объектов Cat
    }
}

const cat = new Cat({//при этом создавая новую переменную от дочернего класса, все его переменные и методы, как и прородительского класса будут также доступны и мы лишь переназначаем им свои значения 
    name: 'Cat',
    age: 10,
    hasTeil: true,
    color: 'black'
...
const box1 = new Box({//теперь создадим пару переменных от дочернего класса с id = 'box1' , id = 'box2', с соответствующими парами ключ-значение, которые уже инициализированы в родительском классе, но с их значениями
    selector: '#box1',
    size: 100,
    color: 'red'
})//для этого после указания требуемых полей новых переменных , в коде html создадим соответствующие теги с соответствующими id, после чего получим в приложении соответствующие боксы (соответсвующие квадраты)
...
class Circle extends Box {//соответственно, мы также можем создать и новый дочерний класс уже от класса второго уровня, который будет уже кругом
    constructor(options) {
        ...
        this.$el.style.borderRadius = '50%'//для этого лишь инициализируем соответствующий аргумент borderRadius для квадрата со значением 50% 
    }
}

const c = new Circle({//а объявив некую переменную как новый объект круг, с соответствующими наследственными полями соответствующими кругу, и их значениями, и добавив соотвествующий тег с соответствующим id в html
    selector: '#circle',
    size: 90,
    color: 'green'
